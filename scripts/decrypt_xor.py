...
def parse_input() -> Tuple[str, bool]:
    ...
    Returns:
        Tuple[str, bool]: Decrypt input using XOR
    """
    ...
    parser = argparse.ArgumentParser(description='Decrypts data using repeating XOR key')
...
def main():
    ...
    if options['invalid'] == True:
        ...
        if options['path']:
            ...
            data = base64.b64decode(options['input'].encode('ascii'))

    decrypted = xor_bytes(repeating_key_stream(options['key'].encode('ascii'), len(data)), data)
